...
const withError = (error?: string) => (error ? "border-destructive focus-visible:ring-destructive/30" : "");
    ...
    <div className={className}>
      ...
      <div className="relative">
        {isTextarea ? (
          ...
          <Input
            name={name}
            type={isPassword ? (showPass ? "text" : "password") : isNumber ? "number" : type === "email" ? "email" : "text"}
            ...
            disabled={disabled}
            readOnly={readOnly}
            className={cn(isPassword && "pr-10", isNumber && "pr-10", withError(error))}
          />
        )}
        ...
        {isNumber && <Hash size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground pointer-events-none" />}
      </div>
...
function DateInput({ value = "", onChange, error, disabled, required, label, className, placeholder = "Sene saýlaň" }: FormInputProps) {
      ...
      >
        <CollapsibleTrigger asChild>
          ...
          >
            ...
            <ChevronsUpDown
              size={14}
              className={cn("text-muted-foreground transition-transform duration-200 shrink-0", open && "text-foreground")}
            />
          </button>
        </CollapsibleTrigger>
    ...
    <div className={cn("relative", className)}>
      ...
      >
        ...
        <CollapsibleContent className="mt-1 rounded-md border border-border bg-card shadow-lg overflow-hidden">
          ...
          <div className="max-h-52 overflow-y-auto py-1">
            {filtered.length === 0 ? (
              <p className="px-3 py-4 text-sm text-muted-foreground text-center">Netije tapylmady</p>
            ) : (
              filtered.map((opt) => {
                const isSelected = opt.value === value;
                return (
...
function FileInput({
  ...
  useEffect(() => {
    ...
    if (fileValue?.type.startsWith("image/")) {
      ...
      timer = setTimeout(() => setPreview(url!), 0);
    } else {
      timer = setTimeout(() => setPreview(null), 0);
    }
    ...
    <div className={className}>
      ...
      >
        {fileValue && isImage && preview ? (
          <>
            <img src={preview} alt="preview" className="w-full max-h-56 object-contain bg-muted/30 p-2" />
            <div className="w-full flex items-center justify-between px-3 py-2 bg-muted/40 border-t border-border">
              <span className="text-xs text-muted-foreground truncate max-w-[200px]">{fileValue.name}</span>
              <Button
                type="button"
                size="icon"
                variant="ghost"
          ...
          <div className="flex flex-col items-center gap-1.5 p-4">
            ...
            <p className="text-xs font-medium text-foreground text-center truncate max-w-[180px]">{fileValue.name}</p>
            <button
              type="button"
              onClick={(e) => {
                e.stopPropagation();
          ...
          <div className="flex flex-col items-center gap-1.5 p-4">
            ...
            <p className="text-xs text-muted-foreground text-center leading-relaxed">{placeholder}</p>
          </div>
        )}
      </div>
