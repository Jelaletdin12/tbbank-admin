  ...
  return (
    <div className="flex flex-col gap-5">
      <h1 className="text-xl font-semibold text-foreground">
        {mode === "create" ? t("operators.createTitle", "Operator dörediň") : t("operators.editTitle", "Operator üýtgetmek")}
      </h1>
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        {/* Username */}
        <div className="grid grid-cols-[220px_1fr] items-start py-4 px-6 border-b border-border">
          <span className="text-xs font-bold text-muted-foreground pt-2">
        ...
        {/* Phone */}
        <div className="grid grid-cols-[220px_1fr] items-start py-4 px-6 border-b border-border">
          <span className="text-sm text-muted-foreground pt-2">{t("operators.fields.phone", "Telefon")}</span>
          <FormInput type="phone" value={form.phone} onChange={(v) => setField("phone", v)} disabled={isPending} className="max-w-lg" />
        </div>

        {/* Email */}
        <div className="grid grid-cols-[220px_1fr] items-start py-4 px-6 border-b border-border">
          <span className="text-sm text-muted-foreground pt-2">{t("operators.fields.email", "E-poçta")}</span>
          <FormInput
            type="email"
            value={form.email}
        ...
        <div className="grid grid-cols-[220px_1fr] items-start py-4 px-6 border-b border-border">
          ...
          <div className="max-w-lg space-y-1">
            ...
            />
            {mode === "edit" && (
              <p className="text-xs text-muted-foreground">{t("operators.fields.passwordEditNote", "Boş goýsaňyz, açar sözi üýtgemez")}</p>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-[220px_1fr] items-center py-4 px-6">
          <span className="text-sm text-muted-foreground">{t("operators.fields.isActive", "Işjeň")}</span>
          <Checkbox
        ...
        <FormActions
          ...
          onCancel={() => navigate(-1)}
          cancelVariant="ghost"
          submitLabel={mode === "create" ? t("operators.createBtn", "Operator döredin") : t("operators.updateBtn", "Üýtgetmeleri sakla")}
