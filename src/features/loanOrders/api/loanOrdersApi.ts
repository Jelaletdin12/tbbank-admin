...
export type LoanOrderStatus = "GARAŞYLÝAR" | "KANAGATLANDYRYLAN" | "RED_EDILDI" | "IŞLENÝÄR";
...
export async function fetchLoanOrders(params: LoanOrdersParams): Promise<LoanOrdersResponse> {
  await delay(600);
  ...
  let filtered = mockStore.filter((item) => {
    ...
    const matchStatus = !params.status ? true : item.status === params.status;
    const matchRegion = !params.region ? true : item.region === params.region;
    const matchBranch = !params.branch ? true : item.branch === params.branch;
...
export async function createLoanOrder(payload: LoanOrderPayload): Promise<LoanOrder> {
  await delay(500);
  const newItem = {
    id: `L-${nextId++}`,
...
export async function updateLoanOrder(id: string, payload: Partial<LoanOrderPayload>): Promise<LoanOrder> {
  await delay(500);
