...
export const loanOrderFormSchema = z.object({
  ...
  homePhone: z
    ...
    .refine(
      (val) => {
        if (!val || val.replace(/\D/g, "").length === 0) return true;
        return VALID_PHONE_PREFIXES.includes(val.replace(/\D/g, "").slice(0, 2));
      },
      { message: "validation.invalidPhonePrefix" },
    )
    .optional(),
...
export const stepSchemas: Record<number, z.ZodType<Partial<LoanOrderFormData>>> = {
  ...
  5: loanOrderFormSchema.pick({
    ...
    passportGivenBy: true,
  }),
  6: loanOrderFormSchema.pick({ phone: true, phoneAdditional: true, homePhone: true }),
  ...
  9: loanOrderFormSchema.pick({
    guarantor1Name: true,
    guarantor1Surname: true,
    guarantor1PassportSerie: true,
    guarantor1PassportNumber: true,
    guarantor1CardNumber: true,
    guarantor1CardName: true,
    guarantor1CardExpMonth: true,
    guarantor1CardExpYear: true,
...
export function validateStep(
  ...
  mode: "create" | "edit",
): Partial<Record<keyof LoanOrderFormData, string>> {
