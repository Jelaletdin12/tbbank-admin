...
export const intlPaymentApi = {
  ...
  delete: (id: string): Promise<void> =>
    apiClient.delete(`/intl-payments/visa-master/${id}`).then((r) => r.data),
}

// ─── Helper ───────────────────────────────────────────────────────────────────

const TEXT_FIELDS: (keyof IntlPaymentCreatePayload)[] = [
  'client_id', 'status', 'note', 'currency_type', 'province', 'branch',
  'passport_first_name', 'passport_last_name', 'phone', 'email', 'home_address',
  'passport_series', 'passport_number', 'payer_full_name', 'payer_account_number',
  'receiver_info',
]
