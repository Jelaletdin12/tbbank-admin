...
export function DistrictForm({ mode, initialData }: DistrictFormProps) {
  ...
  const set = useCallback(
    <K extends keyof DistrictFormData>(key: K) =>
  ...
  const handleSubmit = async () => {
    ...
    if (mode === "create") {
      ...
      updateMutation.mutate({ id: initialData.id, ...payload }, { onSuccess: () => navigate("/settings/location/districts") });
    }
  };
  ...
  return (
    <div className="flex flex-col gap-5">
      <h1 className="text-xl font-semibold text-foreground">
        {mode === "create" ? t("districts.createTitle", "Etrap dörediň") : t("districts.editTitle", "Etrap üýtgetmek")}
      </h1>
      <BentoGrid cols={2}>
        <BentoCard span="full">
        ...
        <BentoCard>
          <div className="flex items-center gap-3 pt-1">
            <span className="text-sm font-semibold text-muted-foreground">{t("districts.fields.isActive", "Işjeň")}</span>
      ...
      <FormActions
        ...
        cancelVariant="ghost"
        submitLabel={mode === "create" ? t("districts.actions.create", "Etrap döret") : t("districts.actions.update", "Täzelemek")}
      />
    </div>
  );
}
