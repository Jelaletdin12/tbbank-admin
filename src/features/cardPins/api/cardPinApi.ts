...
export const cardPinApi = {
  ...
  update: (id: string, payload: CardPinUpdatePayload): Promise<CardPinItem> => {
    ...
    return apiClient
      .post(`/card-pins/${id}`, form, {
        headers: { "Content-Type": "multipart/form-data" },
      })
      .then((r) => r.data);
  },

  delete: (id: string): Promise<void> => apiClient.delete(`/card-pins/${id}`).then((r) => r.data),
};
...
function buildFormData(payload: CardPinCreatePayload): FormData {
  ...
  const fileFields: (keyof CardPinCreatePayload)[] = ["passport_file_1", "passport_file_2", "passport_file_3", "passport_file_4"];
