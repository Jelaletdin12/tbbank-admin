...
function applyMockFilters(
  params: LoanPaidOffLetterListParams,
): PaginatedResponse<LoanPaidOffLetter> {
...
export async function fetchLoanPaidOffLetters(
  params: LoanPaidOffLetterListParams,
): Promise<PaginatedResponse<LoanPaidOffLetter>> {
  if (USE_MOCK) return applyMockFilters(params)

  const { data } = await apiClient.get<PaginatedResponse<LoanPaidOffLetter>>(
