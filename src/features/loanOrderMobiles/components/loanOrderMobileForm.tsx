...
function mapToFormState(order: LoanOrderMobile): FormState {
  return {
    ...
    guarantor1Salary:
      order.guarantor1Salary != null ? String(order.guarantor1Salary) : "",
  };
}

// ─── Step definitions ─────────────────────────────────────────────────────────

// Keys that belong to each step — used for per-step validation
const STEP_FIELDS: Array<Array<keyof FormState>> = [
  // Step 0 — Karz & Lokasiýa
  ["loanType", "loanAmount", "region", "branch"],
  // Step 1 — Şahsy maglumatlar & Pasport
  [
    "firstName",
    "lastName",
    "education",
  ...
  // Step 4 — Faýllar & Status
  ["passportPage1", "passportPage23"],
];
...
function validate(
  ...
  const req = (key: keyof FormState, label: string) => {
    ...
    if (!val || (typeof val === "string" && !val.trim())) {
      allErrors[key] = `${label} — ${t("required") || "hökmany"}`;
    }
  };
  ...
  if (form.phone && form.phone.replace(/\D/g, "").length < 8) {
    allErrors.phone = "Telefon belgisi nädogry";
  }

  if (mode === "create") {
    if (!form.passportPage1) allErrors.passportPage1 = "Pasport (sahypa 1) hökmany";
    if (!form.passportPage23) allErrors.passportPage23 = "Pasport (sahypa 2-3) hökmany";
  }

  // If stepIndex provided, filter to only that step's fields
...
export function LoanOrderMobileForm({
  mode,
  initialData,
  loanOrderId,
}: LoanOrderMobileFormProps) {
  ...
  // Track which steps have been validated (done or error)
  const [stepStatuses, setStepStatuses] = useState<
    Array<"idle" | "active" | "done" | "error">
  ...
  // Build step items for StepBarCards
  const STEP_DEFS = [
    {
      id: "loan",
      title: t("Loan") || "Karz",
      subtitle: t("Type & amount") || "Görnüş & möçber",
      icon: FileText,
    },
    {
      id: "personal",
      title: t("Personal") || "Şahsy",
      subtitle: t("Identity & passport") || "Şahsyýet & pasport",
      icon: User,
    },
    {
      id: "contact",
      title: t("Contact & Job") || "Habarlaşmak & Iş",
      subtitle: t("Phone & employment") || "Telefon & iş",
      icon: Phone,
    },
    {
      id: "card",
      title: t("Card & Guarantor") || "Kart & Zamun",
      subtitle: t("Payment & guarantor") || "Töleg & zamun",
      icon: CreditCard,
    },
    {
      id: "files",
      title: t("Files & Status") || "Faýllar & Status",
      subtitle: t("Documents & review") || "Resminamalar",
      icon: Upload,
    },
  ];

  const stepItems: StepCardItem[] = STEP_DEFS.map((def, i) => ({
    ...def,
  ...
  const goToStep = (index: number) => {
    updateStepStatus(currentStep, stepStatuses[currentStep] === "error" ? "error" : "done");
  ...
  const handleNext = () => {
    const stepErrors = validate(form, t, mode, currentStep);
  ...
  const handlePrev = () => {
    ...
    updateStepStatus(currentStep, stepStatuses[currentStep] === "error" ? "error" : "idle");
    updateStepStatus(prev, "active");
    setCurrentStep(prev);
    window.scrollTo({ top: 0, behavior: "smooth" });
  };
  ...
  const handleSubmit = () => {
    ...
    const allErrors = validate(form, t, mode);
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {currentStep === 4 && (
        <>
          ...
          <Section title={t("Passport files") || "Pasport faýýlar"}>
            {/* Edit modunda mevcut dosyaları göster */}
            {mode === "edit" && initialData && (
              <div className={`${grid4} mb-6`}>
                ...
                {initialData.passportPage89Url && (
                  <div className="flex flex-col gap-1">
                    <span className="text-xs text-muted-foreground">
                      {t("Passport (page 8-9)") || "Pasport (8-9 sahypa)"} —{" "}
                      {t("current") || "häzirki"}
                    </span>
                    <a
                ...
                {initialData.passportPage32Url && (
                  <div className="flex flex-col gap-1">
                    <span className="text-xs text-muted-foreground">
                      {t("Passport (page 32)") || "Pasport (32-nji sahypa)"} —{" "}
                      {t("current") || "häzirki"}
                    </span>
                    <a
      ...
      <FormActions
        ...
        // Prev button: visible from step 1 onward
        onPrev={!isFirstStep ? handlePrev : undefined}
        prevLabel={t("common.prev") || "Yza"}
        // Next button: visible on all steps except the last
