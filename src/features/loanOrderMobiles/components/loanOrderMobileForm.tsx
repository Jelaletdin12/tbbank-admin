...
import { FileText, User, Phone, CreditCard, Upload } from "lucide-react";
...
import type { LoanOrderMobile } from "@/features/loanOrderMobiles/api/loanOrderMobilesApi";
...
const STEPS: StepDef[] = [
  {
    id: "loan",
    titleKey: "loanOrderMobileForm.steps.loan.title",
    titleFallback: "Karz",
    subtitleKey: "loanOrderMobileForm.steps.loan.subtitle",
    subtitleFallback: "Görnüş, möçber & lokasiýa",
    icon: FileText,
    validate: (form, mode) => validateStep(0, form, mode),
  },
  {
    id: "personal",
    titleKey: "loanOrderMobileForm.steps.personal.title",
    titleFallback: "Şahsy & Pasport",
    subtitleKey: "loanOrderMobileForm.steps.personal.subtitle",
    subtitleFallback: "Şahsyýet & pasport",
    icon: User,
    validate: (form, mode) => validateStep(1, form, mode),
  },
  {
    id: "contact",
    titleKey: "loanOrderMobileForm.steps.contact.title",
    titleFallback: "Habarlaşmak & Iş",
    subtitleKey: "loanOrderMobileForm.steps.contact.subtitle",
    subtitleFallback: "Telefon & iş",
    icon: Phone,
    validate: (form, mode) => validateStep(2, form, mode),
  },
  {
    id: "card",
    titleKey: "loanOrderMobileForm.steps.card.title",
    titleFallback: "Kart & Zamun",
    subtitleKey: "loanOrderMobileForm.steps.card.subtitle",
    subtitleFallback: "Töleg & zamun",
    icon: CreditCard,
    validate: (form, mode) => validateStep(3, form, mode),
  },
  {
    id: "files",
    titleKey: "loanOrderMobileForm.steps.files.title",
    titleFallback: "Faýllar & Status",
    subtitleKey: "loanOrderMobileForm.steps.files.subtitle",
...
export function LoanOrderMobileForm({ mode, initialData }: LoanOrderMobileFormProps) {
  ...
  const {
    ...
    clearErrors,
    setError,
  } = useForm<LoanOrderMobileFormData>({
  ...
  const set = useCallback(
    <K extends keyof LoanOrderMobileFormData>(key: K, value: LoanOrderMobileFormData[K]) => {
  ...
  const doSubmit = () => {
    ...
    if (mode === "create") {
      ...
      updateMutation.mutate({ id: initialData.id, payload }, { onSuccess: () => navigate("/loan-order-mobiles") });
    }
  };
