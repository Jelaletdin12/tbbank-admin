...
function applyMockFilters(
  params: LoanRemainingListParams,
): PaginatedResponse<LoanRemaining> {
...
export async function fetchLoanRemaining(
  params: LoanRemainingListParams,
): Promise<PaginatedResponse<LoanRemaining>> {
  if (USE_MOCK) return applyMockFilters(params)

  const { data } = await apiClient.get<PaginatedResponse<LoanRemaining>>(
    '/nova-loan-remaining-orders',
    { params },
  )
  return data
}
