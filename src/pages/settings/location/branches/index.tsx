...
export function BranchesListPage() {
  ...
  const [columnOrder, setColumnOrder] = useState<string[]>([
    'id', 'code', 'name', 'districtName', 'phone', 'isActive', 'actions',
  ])
  ...
  const columns: ColumnDef<Branch>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-sm font-mono text-foreground">{row.original.code}</span>
      ),
      size: 100,
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-sm text-muted-foreground">{row.original.districtName[lang]}</span>
      ),
    },
    {
      accessorKey: 'phone',
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2 justify-end">
          <button
            ...
            onClick={() => navigate(`/settings/location/branches/${row.original.id}`)}
          ...
          <button
            className="p-1.5 rounded hover:bg-muted/60 text-muted-foreground hover:text-foreground transition-colors"
            onClick={() => navigate(`/settings/location/branches/${row.original.id}/edit`)}
...
<div className="bg-card border border-border rounded-xl p-4"> 
      ...
      <DataTableToolbar
        ...
        actionLabel={t('branches.actions.create', 'Şahamça döret')}
        onAction={() => navigate('/settings/location/branches/create')}
      />
