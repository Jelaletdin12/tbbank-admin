...
export function CardTypesListPage() {
  ...
  const columns: ColumnDef<CardType>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <TableActions
          onView={() => navigate(`/settings/card/card-types/${row.original.id}`)}
          onEdit={() => navigate(`/settings/card/card-types/${row.original.id}/edit`)}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">{t("cardTypes.title", "Kart görnüşleri")}</h1>
        </div>
      </div>
      <div className="flex items-center gap-3 justify-between">
        ...
        />
        <CreateButton
          label={t("cardTypes.actions.create", "Kart görnüşi döretdiň")}
          onClick={() => navigate("/settings/card/card-types/create")}
        />
      </div>
      ...
      <DeleteDialog
        ...
        description={t("cardTypes.deleteConfirm", "Bu kart görnüşini öçürmek isleýärsiňizmi?")}
        onConfirm={() => {
