...
} from '@/features/visaMasterSberSettings/hooks/useVisaMasterSettings'
...
export default function VisaMasterSettingsPage() {
  ...
  const columns: ColumnDef<VisaMasterSetting>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-foreground">{row.original.ady}</span>
      ),
    },
    {
      accessorKey: 'yazgy',
      ...
      cell: ({ row }) => (
        <span className="text-foreground line-clamp-1 max-w-xs">{row.original.yazgy}</span>
      ),
    },
    {
      id: 'actions',
      ...
      cell: ({ row }) => (
        <div className="flex items-center justify-end gap-1">
          <Button
            ...
            onClick={() =>
              navigate(`/visa-master-sber-settings/${row.original.id}/edit`)
            }
  ...
  return (
    <div className="p-6">
      ...
      <h1 className="text-2xl font-semibold text-foreground mb-5">
        {t('visaMasterSettings.title', 'Visa/Master, Sber sazlamalar')}
      </h1>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t('visaMasterSettings.deleteDialog.title', 'Pozmak isleýärsiňizmi?')}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {t(
                'visaMasterSettings.deleteDialog.description',
                'Bu amal yzyna gaýtarylyp bilinmez. Sazlama hemişelik pozular.'
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
