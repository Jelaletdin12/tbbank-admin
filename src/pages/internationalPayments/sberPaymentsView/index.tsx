...
import { Trash2, Edit, Search, Download, Clock } from 'lucide-react'
...
function InfoRow({ 
  ...
  return (
    <div className="flex items-start py-2.5 border-b border-border last:border-b-0">
      <span className="w-64 shrink-0 text-sm text-muted-foreground">{label}</span>
      <span className={cn(
        'flex-1 text-sm',
  ...
  if (!order) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] text-muted-foreground">
        <p>Toleg tapylmady</p>
        <Button variant="outline" onClick={() => navigate('/sber-payments')} className="mt-4">
          Yza
        </Button>
      </div>
    )
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <Section title="Status">
        <div className="px-4">
          ...
          <InfoRow label="Tolenen (Sul ay)" value={<PaidStatusBadge status={order.paidStatus} />} />
      ...
      <Section title="Toleg">
        <div className="px-4">
          ...
          />
          <InfoRow label="Toleg ugradyjynyn goyum hasaby" value={order.accountNumber} />
        </div>
        ...
        <div className="px-4 py-3 bg-muted/30 border-t border-border">
          ...
          <div className="grid grid-cols-3 gap-4 text-sm">
            ...
            <div>
              <span className="text-muted-foreground">Ady Familiyasy Atasynyn ady</span>
      ...
      <div className="space-y-3">
        ...
        <div className="bg-card rounded-lg border border-border">
          {order.documents?.slice(6).map((doc) => (
            <DocumentItem
              key={doc.id}
              label={doc.label}
              fileName={doc.name}
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          <h2 className="text-base font-semibold text-foreground">Toleg taryhy</h2>
        ...
        <div className="bg-card rounded-lg border border-border">
          ...
          <div className="flex items-center justify-center py-12 text-muted-foreground">
            <div className="text-center">
              ...
              <p className="text-sm">Berlen kriteriyalara Toleg gabat gelmedi.</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-card rounded-lg border border-border overflow-hidden">
          ...
          {order.activityLog?.length ? (
            order.activityLog.map((log) => (
              <div key={log.id} className="grid grid-cols-6 gap-4 px-4 py-3 border-t border-border text-sm">
                <span className="text-primary font-medium">{log.id.split('-')[1] ?? '45384'}</span>
                ...
                <span className="text-muted-foreground">
                  {format(new Date(log.createdAt), 'dd/MM/yyyy, HH:mm:ss')} GMT+5
                </span>
              </div>
            ))
          ) : (
