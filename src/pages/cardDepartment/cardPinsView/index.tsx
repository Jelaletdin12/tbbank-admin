    ...
    <div className="mx-auto flex flex-col gap-6">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => navigate(`/card-pins/${id}/edit`)}>
            <Pencil size={15} />
          </Button>
        </div>
      </div>
      ...
      <BentoGrid cols={3}>
        <BentoCard title={t("cardPin.metaSection", "Esasy maglumatlar")}>
          ...
          <InfoRow label={t("cardPin.createdAt", "Döredilen wagty")} value={data.created_at} />
          <InfoRow label={t("cardPin.status", "Status")}>
            <CardPinStatusBadge status={data.status} />
          </InfoRow>
          <InfoRow label={t("cardPin.note", "Bellik")} value={data.note || undefined} />
          <InfoRow label={t("cardPin.createdBy", "Sargyt eden")} value={data.created_by_label} isLink />
        </BentoCard>

        <BentoCard title={t("cardPin.cardSection", "Kart")}>
          <InfoRow label={t("cardPin.cardNumber", "Kart belgisi")} value={data.card_number} />
          <InfoRow label={t("cardPin.cardExpireMonth", "Kart Möhleti (aý)")} value={null} />
          <InfoRow label={t("cardPin.cardExpireYear", "Kart Möhleti (ýyl)")} value={null} />
        </BentoCard>

        <BentoCard title={t("cardPin.locationSection", "Lokasiýa")}>
          <InfoRow label={t("cardPin.province", "Welaýat")} value={data.province_label} />
          <InfoRow label={t("cardPin.branch", "Şahamça")} value={data.branch_label} isLink />
        </BentoCard>
      </BentoGrid>
      ...
      <BentoGrid cols={2}>
        <BentoCard title={t("cardPin.personalSection", "Şahsy maglumatlar")}>
          <InfoRow label={t("cardPin.fullName", "Doly ady")} value={`${data.last_name} ${data.first_name} ${data.father_name}`} />
          <InfoRow label={t("cardPin.birthDate", "Doglan güni")} value={data.birth_date} />
          <InfoRow label={t("cardPin.phone", "Telefon")} value={`+${data.phone}`} />
        </BentoCard>

        <BentoCard title={t("cardPin.passportSection", "Pasport")}>
          <InfoRow label={t("cardPin.passportSeries", "Pasport seriýasy")} value={data.passport_series} />
          <InfoRow label={t("cardPin.passportNumber", "Pasport belgisi")} value={data.passport_number} />
        </BentoCard>
      </BentoGrid>
      ...
      <BentoGrid cols={4}>
        <BentoCard title={t("cardPin.passportFile1", "Sahypa 1")}>
          <PassportImage label={t("cardPin.passportFile1", "Pasport (sahypa 1)")} url={data.passport_file_1} />
        </BentoCard>
        <BentoCard title={t("cardPin.passportFile2", "Sahypa 2-3")}>
          <PassportImage label={t("cardPin.passportFile2", "Pasport (2-3-nji sahypa)")} url={data.passport_file_2} />
        </BentoCard>
        <BentoCard title={t("cardPin.passportFile3", "Sahypa 8-9")}>
          <PassportImage label={t("cardPin.passportFile3", "Pasport (8-9 sahypa)")} url={data.passport_file_3} />
        </BentoCard>
        <BentoCard title={t("cardPin.passportFile4", "Sahypa 32")}>
          <PassportImage label={t("cardPin.passportFile4", "Pasport (32-nji sahypa)")} url={data.passport_file_4} />
        </BentoCard>
      </BentoGrid>
    </div>
  );
