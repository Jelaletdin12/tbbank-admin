...
export default function CardPinsPage() {
  ...
  const columns: ColumnDef<CardPinItem>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => <span className="text-sm font-medium text-primary">{row.original.card_type_label}</span>,
    },
    {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 justify-between">
        ...
        />
        <CreateButton label={t("cardPin.createBtn", "Kart pin bukja dörediň")} onClick={() => navigate("/card-pins/create")} />
      </div>
      <div className="bg-card border border-border rounded-xl p-4">
        <DataTableToolbar
          ...
          onFilterChange={(fieldId, value) => {
            if (fieldId === "status") setStatusFilter(value as CardPinStatus | "");
            setPage(1);
          }}
          onFilterReset={() => {
            setStatusFilter("");
      ...
      <DeleteDialog
        ...
        title={t("cardPin.deleteTitle", "Pozmak isleýärsiňizmi?")}
        description={`${t("cardPin.deleteDesc", "Bu amal yzyna dolanyp bolmaz.")} ${deleteTarget?.id ?? ""}`}
        onConfirm={() => {
          if (deleteTarget) {
