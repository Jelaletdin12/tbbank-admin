...
const STATUS_BADGE: Record<CardRequisiteStatus, { label: string; className: string }> = {
  pending:  { label: 'Garaşylýar', className: 'bg-yellow-500/15 text-yellow-500 border-yellow-500/20' },
  approved: { label: 'Tassyklanan', className: 'bg-emerald-500/15 text-emerald-500 border-emerald-500/20' },
  rejected: { label: 'Ret edilen',  className: 'bg-red-500/15 text-red-500 border-red-500/20' },
}
...
export default function CardRequisitesPage() {
  ...
  const { data, isLoading } = useCardRequisites({ page, per_page: perPage, search })
  ...
  const columns: ColumnDef<CardRequisite>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-primary font-medium">{row.original.card_type}</span>
      ),
    },
    {
      id: 'created_at',
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-foreground">{row.original.province_name ?? '—'}</span>
      ),
    },
    {
      id: 'branch_name',
      ...
      cell: ({ row }) => (
        <span className="text-primary font-medium">{row.original.branch_name ?? '—'}</span>
      ),
    },
    {
      id: 'first_name',
  ...
  return (
    <div className="space-y-4">
      ...
      <DataTableToolbar
        ...
        actionLabel={t('Create card requisite order', 'Kart rekwiziti üçin sargyt dörediň')}
