...
import { useLoanOrderMobiles, useDeleteLoanOrderMobile } from "@/features/loanOrderMobiles/hooks/useLoanOrderMobiles";
...
export default function LoanOrderMobilesPage() {
  ...
  const columns = useMemo<ColumnDef<LoanOrderMobile>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <TableActions
            onView={() => navigate(`/loan-order-mobiles/${row.original.id}`)}
            onEdit={() => navigate(`/loan-order-mobiles/${row.original.id}/edit`)}
  ...
  const toggleableColumns = useMemo(() => COLUMN_IDS.map((id) => ({ id, label: t(`loanOrderMobiles.columns.${id}`, id) })), [t]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold text-foreground">{t("loanOrderMobiles.title", "Karz sargytlary")}</h1>
      </div>
      ...
      <div className="flex items-center gap-3 justify-between">
        ...
        />
        <CreateButton
          label={t("loanOrderMobiles.createButton", "Karz sargyt dörediň")}
          onClick={() => navigate("/loan-order-mobiles/create")}
        />
      </div>
