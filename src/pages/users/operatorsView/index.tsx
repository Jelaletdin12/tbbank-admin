...
export default function OperatorDetailPage() {
  ...
  // Map audit logs to AuditLog component format
  ...
  return (
    <div className="p-6">
      ...
      <CollapsibleSection
        ...
        onAction={() => {/* open role assign modal */}}
      >
        <DataTableToolbar
      ...
      <CollapsibleSection
        ...
        onAction={() => {/* open permission modal */}}
      >
        <DataTableToolbar
        ...
        {(operator.permissions ?? []).length === 0 ? (
          <EmptyState label={t('operators.noPermissions', 'Berlen kriterýalara Rugsat gabat gelmedi.')} actionLabel={t('operators.addPermission', 'Rugsat birikdiriň')} onAction={() => {}} />
        ) : null}
      </CollapsibleSection>
      ...
      >
        {(operator.loanLimits ?? []).length === 0 ? (
          <EmptyState label={t('operators.noLoanLimits', 'Berlen kriterýalara Karz sargyt gabat gelmedi.')} actionLabel={t('operators.addLoanLimit', 'Karz sargyt döredin')} onAction={() => {}} />
        ) : null}
      </CollapsibleSection>
      ...
      >
        {(operator.cardLimits ?? []).length === 0 ? (
          <EmptyState label={t('operators.noCardLimits', 'Berlen kriterýalara Kart sargyt gabat gelmedi.')} actionLabel={t('operators.addCardLimit', 'Kart sargyt döredin')} onAction={() => {}} />
        ) : null}
      </CollapsibleSection>
...
function CollapsibleSection({ title, actionLabel, onAction, children }: CollapsibleSectionProps) {
  ...
  return (
    <div className="mb-6">
      <div
        className="flex items-center justify-between mb-2 cursor-pointer select-none w-fit gap-2"
        onClick={() => setOpen((v) => !v)}
      >
